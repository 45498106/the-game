module.exports = class Game {
    ...
    collision(character, item) {
        ...
        if (
            ((charPos.x <= itemPos.x && charPos.x + 16 >= itemPos.x) ||
                (charPos.x >= itemPos.x && charPos.x <= itemPos.x + 16)) &&
            ((charPos.y <= itemPos.y && charPos.y + 16 >= itemPos.y) ||
                (charPos.y >= itemPos.y && charPos.y <= itemPos.y + 16))
        ) {
            return true;
        }
    }
